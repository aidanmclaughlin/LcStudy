...
def test_session_create_request_valid():
    request = SessionCreateRequest(
        maia_level=1500, player_color=PlayerColor.WHITE, custom_fen=None
    )
...
def test_session_create_request_invalid_level():
    with pytest.raises(ValidationError) as exc_info:
        SessionCreateRequest(maia_level=1000, player_color=PlayerColor.WHITE)
...
def test_session_create_request_invalid_level_range():
    with pytest.raises(ValidationError):
        SessionCreateRequest(maia_level=2000, player_color=PlayerColor.WHITE)


def test_move_request_valid():
    request = MoveRequest(move="e2e4", client_validated=False)
