...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def illegal_move_handler(_: Request, exc: IllegalMoveError):
        return JSONResponse(
            status_code=400,
            content={
                "error": {"code": "illegal_move", "message": str(exc) or "Illegal move"}
            },
        )
    ...
    async def game_finished_handler(_: Request, exc: GameFinishedError):
        return JSONResponse(
            ...
            content={
                "error": {
                    "code": "game_finished",
                    "message": str(exc) or "Game is finished",
                }
            },
        )
        ...
        return JSONResponse(
            ...
            content={
                "error": {
                    ...
                    "message": str(exc) or "Engine not available",
                }
            },
        )
    ...
    async def data_error_handler(_: Request, exc: DataError):
        return JSONResponse(
            status_code=500,
            content={
                "error": {"code": "data_error", "message": str(exc) or "Data error"}
            },
        )
