...
import chess
import chess.engine
from ..engines import Lc0Engine, EngineConfig, find_lc0, nets_dir, info_to_lines
from ..exceptions import EngineError, EngineNotFoundError, EngineAnalysisError
...
class LeelaEngine(EngineInterface):
    ...
    def analyze(self, board: chess.Board, nodes: int) -> List[dict]:
        logger.debug(f"Starting analysis with {nodes} nodes for position: {board.fen()}")
        with self._lock:
            try:
                logger.debug("Calling engine.analyse...")
                infos = self.engine.analyse(board, nodes=nodes)
                logger.debug(f"Engine returned {len(infos) if isinstance(infos, list) else 1} analysis result(s)")
...
class EngineService:
        ...
        with self._engine_lock:
            if session_id not in self._leela_engines:
                ...
                try:
                    ...
                    logger.info(f"Leela config: exe={config.exe}, weights={config.weights}")
                    self._leela_engines[session_id] = LeelaEngine(config)
                    logger.info(f"Leela engine created successfully for session {session_id}")
                except Exception as e:
                    logger.error(f"Failed to create Leela engine for session {session_id}: {e}")
                    raise
            else:
        ...
        with self._engine_lock:
            if level not in self._maia_engines:
                ...
                try:
                    ...
                    logger.info(f"Maia config: exe={config.exe}, weights={config.weights}")
    ...
    def _create_leela_config(self) -> EngineConfig:
        ...
        if not best_net.exists():
            ...
            logger.info(f"Available files in {nets}: {list(nets.glob('*.pb.gz')) if nets.exists() else 'Directory does not exist'}")
